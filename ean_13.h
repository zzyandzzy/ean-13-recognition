...
public:
    ...
    static cv::Rect find(std::string path);
...
private:
    ...
    static barcode_info get_min_unit(const uchar *line, int width);

    static std::string decode(const std::vector<float> &barcode_data);

    static std::vector<float> convert(const uchar *line, barcode_info info);
